 ...
 * @license MIT
 * @author Yuki AOI
 * @version 2.8.0 // 429エラー(レート制限)時の自動リトライを実装、MAX_RETRIESをデフォルト3に変更
...
const VertexAiUtil = {
  ...
  DEBUG_MODE: false,
  MAX_RETRIES: 3, // レート制限(429)やサーバーエラー(5xx)時の最大リトライ回数（合計4回試行）
  GEMINI_MODELS: {
  ...
  ConfigurationError: class extends Error {
    constructor(message) {
      super(message);
      this.name = "VertexAiUtil.ConfigurationError";
    }
  },
  /**
   * @class
  ...
  ApiError: class extends Error {
    constructor(message, statusCode, responseBody) {
      super(message);
      this.name = "VertexAiUtil.ApiError";
      this.statusCode = statusCode;
      this.responseBody = responseBody;
      this.statusCodeDescription =
        VertexAiUtil._getStatusCodeDescription(statusCode);
    }
  },
  ...
  /**
   * @private
   * HTTPステータスコードに対応する説明を返します。
   */
  _getStatusCodeDescription: function (statusCode) {
    const descriptions = {
      400: "※リクエストの形式エラー（プロンプトやペイロードの問題）",
      401: "※認証エラー（サービスアカウントキーの問題）",
      403: "※権限エラー（Vertex AI APIの権限不足）",
      404: "※リソースが見つかりません（モデル名やプロジェクトIDの誤り）",
      429: "※レート制限超過（API呼び出し頻度が高すぎる）",
      500: "※サーバーエラー（Google側の一時的な問題）",
      503: "※サービス一時停止（Google側のメンテナンスまたは障害）",
    };
    return descriptions[statusCode] || "※不明なエラー";
  },
  ...
  Option: class {
    constructor(initialValues = {}) {
      ...
      this._params = {};
      if (initialValues) {
        if (initialValues.customInstruction)
          this.setCustomInstruction(initialValues.customInstruction);
        if (initialValues.temperature != null)
          this.setTemperature(initialValues.temperature);
        if (initialValues.maxOutputTokens)
          this.setMaxOutputTokens(initialValues.maxOutputTokens);
    ...
    setCustomInstruction(instructionText) {
      if (typeof instructionText !== "string")
        throw new VertexAiUtil.ConfigurationError(
          "カスタム指示は文字列である必要があります。"
        );
    ...
    setTemperature(temperature) {
      if (
        ...
        throw new VertexAiUtil.ConfigurationError(
          "temperatureは0.0から2.0の範囲の数値である必要があります。"
        );
      }
    ...
    setMaxOutputTokens(maxTokens) {
      if (
        ...
        throw new VertexAiUtil.ConfigurationError(
          "maxOutputTokensは正の整数である必要があります。"
        );
  ...
  /**
   * レート制限(429)やサーバーエラー(5xx)発生時の最大リトライ回数を設定します。
   * @param {number} count - 0以上の整数。
   */
  setMaxRetries: function (count) {
    if (typeof count !== "number" || count < 0 || !Number.isInteger(count)) {
      throw new this.ConfigurationError(
        "リトライ回数は0以上の整数である必要があります。"
      );
    }
  ...
  generateText: function (prompt, opts) {
    ...
    if (!responseText && data?.candidates?.[0]?.finishReason) {
      throw new Error(
        `Gemini APIからテキスト応答がありませんでした。終了理由: ${data.candidates[0].finishReason}`
      );
    }
  ...
  /**
   * 【単一添付】テキストと添付ファイル（画像、PDFなど）をAIに渡し、応答を生成します。
   * @param {string} prompt - AIへの指示プロンプト。
  ...
  generateTextWithAttachment: function (prompt, attachmentBlob, opts) {
    // 複数形メソッドのラッパーとして動作
    return this.generateTextWithAttachments(prompt, [attachmentBlob], opts);
  },

  /**
   * 【複数添付】テキストと複数の添付ファイル（画像、PDFなど）をAIに渡し、応答を生成します。
   * @param {string} prompt - AIへの指示プロンプト。
   * @param {GoogleAppsScript.Base.Blob[]} attachmentBlobs - Blobオブジェクトの配列。
   * @param {VertexAiUtil.Option} [opts] - オプション設定。
   * @returns {string} AIによって生成されたテキスト応答。
   */
  generateTextWithAttachments: function (prompt, attachmentBlobs, opts) {
    const params = opts ? opts._params : {};

    const textPart = {
      text: prompt,
    };
    const fileParts = (attachmentBlobs || []).map((blob) => {
      return {
        inlineData: {
  ...
  /**
   * 【単一添付】テキストと添付ファイル（画像、PDFなど）を分析し、JSON形式の応答を生成します。
   * @param {string} prompt - AIへの指示プロンプト。
   * @param {GoogleAppsScript.Base.Blob} attachmentBlob - Blobオブジェクト。
   * @param {VertexAiUtil.Option} [opts] - オプション設定。
   * @returns {object} AIからの応答をパースしたJSONオブジェクト。
   */
  generateJsonWithAttachment: function (prompt, attachmentBlob, opts) {
    // 複数形メソッドのラッパーとして動作
    return this.generateJsonWithAttachments(prompt, [attachmentBlob], opts);
  },

  /**
   * 【複数添付】テキストと複数の添付ファイル（画像、PDFなど）を分析し、JSON形式の応答を生成します。
   * @param {string} prompt - AIへの指示プロンプト。
   * @param {GoogleAppsScript.Base.Blob[]} attachmentBlobs - Blobオブジェクトの配列。
   * @param {VertexAiUtil.Option} [opts] - オプション設定。
   * @returns {object} AIからの応答をパースしたJSONオブジェクト。
   */
  generateJsonWithAttachments: function (prompt, attachmentBlobs, opts) {
    const params = opts ? opts._params : {};
    ...
    const sendMessage = (prompt) => {
      ...
      if (sessionOpts.system) {
        payload.systemInstruction = {
          ...
          parts: [
            {
              text: String(sessionOpts.system),
            },
          ],
        };
      }
      const callParams = {
        ...
        region: sessionOpts.region,
        temperature: sessionOpts.temperature,
        maxOutputTokens: sessionOpts.maxOutputTokens,
      };
      const data = this._vertexCall({ ...payload, ...callParams });
      const responseContent = data?.candidates?.[0]?.content;
      if (responseContent) {
        history.push(responseContent);
        return responseContent.parts
          .map((p) => p.text)
          .join("")
          .trim();
      } else {
        ...
        throw new Error(
          `Gemini APIから応答がありませんでした。終了理由: ${stopReason}`
        );
      }
    };
    return {
  ...
  countTokens: function (text, opts = {}) {
    const { token, projectId } = this._getSaAccessToken();
    ...
    if (responseCode !== 200) {
      throw new Error(
        `Token count APIエラー ${responseCode}: ${res.getContentText()}`
      );
    }
  ...
  generateTextStream: function () {
    throw new Error(
      "generateTextStreamはGoogle Apps Scriptの技術的制約により実装されていません。リアルタイム応答が必要な場合はHtmlServiceの利用を検討してください。"
    );
  },
  ...
  _vertexCall: function ({
    ...
    if (!prompt && !contents)
      throw new this.ConfigurationError(
        "内部エラー: prompt または contents が必要です"
      );
    const { token, projectId } = this._getSaAccessToken();
    ...
    for (let i = 0; i <= this.MAX_RETRIES; i++) {
      if (this.DEBUG_MODE) {
        Logger.log(
          `[DEBUG] Vertex AI Request (Attempt ${i + 1} of ${
            this.MAX_RETRIES + 1
      ...
      try {
        ...
        throw new this.ApiError(
          `UrlFetchAppでネットワークエラーが発生しました: ${e.message}`,
          0,
          e.toString()
        );
      }
      ...
      if (this.DEBUG_MODE) {
        Logger.log(
          `[DEBUG] Vertex AI Response (Attempt ${
            i + 1
          }):\nStatus: ${responseCode}\nBody: ${responseBody}`
        );
      }
      ...
      const shouldRetry =
        (responseCode === 429 || responseCode >= 500) && i < this.MAX_RETRIES;

      if (shouldRetry) {
        // 429の場合は長めの待機時間、500系は指数バックオフ
        let waitTime;
        if (responseCode === 429) {
          // レート制限の場合: 60秒 + ランダム(0-30秒)
          waitTime = 60000 + Math.floor(Math.random() * 30000);
        } else {
          // サーバーエラーの場合: 指数バックオフ (1秒, 2秒, 4秒...)
          waitTime = 2 ** i * 1000 + Math.floor(Math.random() * 1000);
        }

        Logger.log(
          `API${
            responseCode === 429 ? "レート制限" : "サーバーエラー"
          }(ステータス: ${responseCode})を検知。${Math.round(
            waitTime / 1000
          )}秒後にリトライします (試行 ${i + 1}/${this.MAX_RETRIES + 1}回目)`
        );
   ...
   * @private
   */
  _getSaAccessToken: function () {
    const props = PropertiesService.getScriptProperties();
    const keyStr = props.getProperty("SA_KEY_JSON");

    if (!keyStr) {
      throw new this.ConfigurationError(
        "スクリプトプロパティ `SA_KEY_JSON` が設定されていません。"
      );
    }
    ...
    try {
      ...
      throw new this.ConfigurationError(
        "スクリプトプロパティ `SA_KEY_JSON` は有効なJSON形式ではありません。貼り付けミスがないか確認してください。"
      );
    }
    ...
    if (errorMessages.length > 0) {
      throw new this.ConfigurationError(
        "スクリプトプロパティ `SA_KEY_JSON` の内容に不備があります:\n- " +
          errorMessages.join("\n- ")
      );
    }
    ...
    const { tokenUrl, scope } = this._oauthConfig();
    ...
    if (!svc.hasAccess()) {
      throw new Error(
        "サービスアカウントのトークン取得に失敗しました: " + svc.getLastError()
      );
    }
    return {
   ...
   * @private
   */
  _oauthConfig: function () {
    const p = PropertiesService.getScriptProperties();
    return {
      tokenUrl:
        p.getProperty("GCP_TOKEN_URL") || "https://oauth2.googleapis.com/token",
      scope:
        p.getProperty("GCP_SCOPES") ||
...
function testSimpleGenerateText() {
  ...
  try {
    const prompt =
      "Google Apps Scriptとは何ですか？1文で簡潔に説明してください。";
...
function testGenerateJson() {
  ...
  try {
    const prompt =
      "次の人物情報をJSON形式で抽出してください。「日本の首都、東京に住む山田太郎は、Googleでソフトウェアエンジニアとして働いています。」";
...
function testChatFunction() {
  Logger.log("\n--- (4) チャット機能 テスト開始 ---");
  try {
    const chatOptions = new VertexAiUtil.Option().setCustomInstruction(
      "あなたは関西弁を話す親切なアシスタントやで。"
    );
...
function testGenerateTextWithAttachment() {
  ...
  try {
    ...
    if (!fileId) {
      Logger.log(
        "⚠️ テストをスキップしました。スクリプトプロパティに `TEST_FILE_ID` を設定してください。"
      );
      return;
    }
